...
public class EmployeeService {
    ...
    public ResponseEntity<List<EmployeeDTO>> findEmployessBySectorId(ObjectId sectorId) {
        if(sectorId == null)
            return ResponseEntity.badRequest().build();
        var dbEmployees = repository.findEmployeesBySectorId(sectorId);
        if(dbEmployees.isEmpty())
            return ResponseEntity.notFound().build();
        var employessDTO = dbEmployees.get().stream().map(employee -> {
            return new EmployeeDTO(employee);
        }).collect(Collectors.toList());
        return ResponseEntity.ok(employessDTO);
    }
